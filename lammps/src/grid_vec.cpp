...
void GridVec::setup()
{
  ...
  //std::cout << "grid->sublo: " <<grid->sublo[0] << " " << grid->sublo[1] << " " << grid->sublo[2] <<std::endl;
  //std::cout << "grid->subhi: " <<grid->subhi[0] << " " << grid->subhi[1] << " " << grid->subhi[2] <<std::endl;

  //std::cout << "grid->subbox: " <<grid->subbox[0] << " " << grid->subbox[1] << " " << grid->subbox[2] <<std::endl;
   // setup mask
  int *mask = grid->mask;
  ...
  double ccc = 0; 
  std::vector<int> tmpcells;
  for (int j = 0; j < atom->nlocal; j++) {
    if (atom->type[j] == 5 ){
      int cell = grid->cell(atom->x[j]);
        mask[cell]  |= GHOST_MASK;
        ccc+= 1;
        tmpcells.push_back(cell);
    }
  }
