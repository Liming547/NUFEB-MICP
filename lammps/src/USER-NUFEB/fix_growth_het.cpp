...
{
  if (narg < 11)
    error->all(FLERR, "Illegal fix nufeb/growth/het command1");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "growth") == 0) {
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg], "Cmass") == 0){
      Cmass = utils::numeric(FLERR,arg[iarg+1],true,lmp);
      iarg += 2;
    }  else if (strcmp(arg[iarg], "Nmass") == 0){
      Nmass = utils::numeric(FLERR,arg[iarg+1],true,lmp);
      iarg += 2;
    } else {
      error->all(FLERR, "Illegal fix nufeb/growth/het command2");
    }
  }
}
...
void FixGrowthHET::update_atoms()
{
  ...
  double **conc = grid->conc;
  double **oldgrowthrate = grid->growth[0];
  ...
  for (int i = 0; i < grid->ncells; i++) {
    ...
    double tmp5 = 1 / 2.86 * maintain * anoxic * conc[ino3][i] / (no3_affinity + conc[ino3][i]) * o2_affinity / (o2_affinity + conc[io2][i]);
    double tmp6 = 1 / 1.17 * maintain * anoxic * conc[ino2][i] / (no2_affinity + conc[ino2][i]) * o2_affinity / (o2_affinity + conc[io2][i]);
    oldgrowthrate[i][0] = grid->growth[igroup][i][0];
  ...
  for (int i = 0; i < atom->nlocal; i++) {
    if (atom->mask[i] & groupbit) {
      ...
      outer_mass[i] = four_thirds_pi *
	    (outer_radius[i] * outer_radius[i] * outer_radius[i] - radius[i] * radius[i] * radius[i]) *
      ...
      grid->conc[0][cell] += std::max(0.0,Cmass * pow(rmass[i] * 1e15, Nmass) * dt );
