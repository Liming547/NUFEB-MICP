...
namespace plb {
  ...
  public:
    ...
    T getLbPosition(T const physL) const { return physL / l_p / param.getDeltaX(); };
	...
	T getLbdVoverdt(T const physVol) const { return param.getDeltaT() * t_p * 3.0 * nu_p * physVol / param.getDeltaX() / param.getDeltaX() / param.getDeltaX() / l_p / l_p / l_p;};
	T getLbVol(T const physVol) const { return pow(physVol,2/3) / param.getDeltaX() / l_p;};
	
	T getdtoverdtcollision() const { return param.getDeltaT() * t_p * 6.0 * nu_p / rho_p / param.getDeltaX() / param.getDeltaX() / l_p / l_p;};
	//T getdtoverdtcollision() const { return param.getTau();};
	plint getLbResolution() const { return param.getResolution();};
	T getLbMass(T const physMass) const { return physMass/pow(param.getDeltaX(),3)/rho_p/l_p/l_p/l_p;};
	T getPhysForce2D(T const lbForce) const { return lbForce * forceFactor/ param.getDeltaX() / l_p; };
    T getLbForce(T const physForce) const { return physForce / forceFactor; };
